...
export function matchService(message) {
  ...
  // 1) Точное совпадение по полностью нормализованному ключу
  ...
  // 2) Частичное совпадение по первому слову
  for (const key of Object.keys(services)) {
    const keyNorm = normalize(key);
    const base = keyNorm.split(" ")[0];
...
export function randomReply(type) {
  const replies = {
    ...
    smalltalkLite: [
      ...
      "Хочешь, покажу новинки сезона?"
    ],
    styleTalk: [
      "О, нюд — всегда в моде! 💅",
      "Блёстки добавят огня 🔥 Хочешь покажу примеры?",
      "«Кошачий глаз» — шикарный выбор 😻"
    ],
    about: [
      "Я — твой маникюрный гид! Помогу выбрать и записать ✨"
    ]
  };
